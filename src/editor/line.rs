...
/// A grapheme index is a position in the line, where a grapheme is a unicode character.
...
impl Line {
  ...
  fn get_replacement_character(for_str: &str) -> Option<char> {
    ...
    match for_str {
      ...
      // Visible whitespace (non-zero width)
      _ if width > 0 && for_str.trim().is_empty() => Some('␣'),

      // Single-character zero-width cases
      _ => {
  ...
  /// Inserts a character into the line, or appends it at the end if at is greater then grapheme length of the string
