...
impl View {
  ...
  fn insert_char(&mut self, character: char) {
    ...
    // Insert new char in the buffer at text_location
  ...
  /// Center current location in center of view
  ...
  // Ensures self.location.idx points to a valid line index by snapping it to the bottom most line if appropriate.
  // Doesn't trigger scrolling.
  ...
  /// Start the search from the given location
  fn search_from(&mut self, from: Location) {
    // Get the search info refrence if available
    if let Some(search_info) = self.search_info.as_ref() {
      ...
      if let Some(location) = self.buffer.search(query, from) {
        // Set the search info refrence to the new location
        self.text_location = location;
        self.center_text_location();
      }
    } else {
      // If there is no search info, then panic
